part of 'cupertino_app.dart';

/// A [CupertinoApp.router] wrapper that integrates with a [Store].
///
/// The Cupertino counterpart to [ComposableMaterialApp]. Observes
/// [CupertinoAppState] from your app state and automatically rebuilds
/// when locale or brightness changes. Optionally listens to platform
/// events via the [observer] mixin.
///
/// ## Example
///
/// ```dart
/// import 'package:composable_architecture_flutterw/cupertino.dart';
///
/// ComposableCupertinoApp<AppState, AppAction>(
///   title: 'My App',
///   store: appStore,
///   toCupertinoAppState: (state) => state.cupertinoAppState,
/// );
/// ```
class ComposableCupertinoApp<AppState, AppAction> extends StatefulWidget {
  /// The title passed to [CupertinoApp].
  final String title;

  /// The application store.
  final Store<AppState, AppAction> store;

  /// Maps [CupertinoAppState] to a [CupertinoThemeData] for theming.
  final CupertinoThemeData Function(CupertinoAppState appState)? toThemeData;

  /// Projects [CupertinoAppState] from the application state.
  final CupertinoAppState Function(AppState) toCupertinoAppState;

  /// Supported locales passed to [CupertinoApp].
  final Iterable<Locale> supportedLocales;

  /// Localization delegates passed to [CupertinoApp].
  final Iterable<LocalizationsDelegate<dynamic>>? localizationsDelegates;

  /// Optional mixin that maps platform events to store actions.
  final WidgetBindingReducer<AppAction>? observer;

  /// Router configuration for [CupertinoApp.router].
  final RouterConfig<Object>? routerConfig;

  /// Optional builder wrapping the [CupertinoApp]'s navigator.
  final Widget Function(BuildContext, Widget?)? builder;

  /// Called after the first frame is rendered.
  final Function()? onAppStart;

  /// Locale list resolution callback.
  final LocaleListResolutionCallback? localeListResolutionCallback;

  /// Locale resolution callback.
  final LocaleResolutionCallback? localeResolutionCallback;

  /// Widget shown while the app is loading.
  final Widget? onLoadChild;

  /// Creates a [ComposableCupertinoApp].
  const ComposableCupertinoApp({
    super.key,
    required this.title,
    required this.store,
    required this.toCupertinoAppState,
    this.routerConfig,
    this.builder,
    this.observer,
    this.localizationsDelegates,
    this.toThemeData,
    this.supportedLocales = const [Locale('en')],
    this.onAppStart,
    this.onLoadChild,
    this.localeListResolutionCallback,
    this.localeResolutionCallback,
  });

  @override
  State<ComposableCupertinoApp<AppState, AppAction>> createState() =>
      _ComposableCupertinoAppState<AppState, AppAction>();
}

class _ComposableCupertinoAppState<AppState, AppAction> extends State<ComposableCupertinoApp<AppState, AppAction>>
    with WidgetsBindingObserver {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addObserver(this);
    SchedulerBinding.instance.addPostFrameCallback((_) {
      widget.onAppStart?.call();
    });
  }

  @override
  void dispose() {
    WidgetsBinding.instance.removeObserver(this);
    super.dispose();
  }

  @override
  void didChangeLocales(List<Locale>? locales) {
    var action = widget.observer?.didChangeLocales(locales);
    if (action != null) {
      widget.store.send(action);
    }
  }

  @override
  void didChangePlatformBrightness() {
    var action = widget.observer?.didChangePlatformBrightness(
      View.of(context).platformDispatcher.platformBrightness,
    );
    if (action != null) {
      widget.store.send(action);
    }
  }

  @override
  void didChangeAccessibilityFeatures() {
    var action = widget.observer?.didChangeAccessibilityFeatures();
    if (action != null) {
      widget.store.send(action);
    }
  }

  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    var action = widget.observer?.didChangeAppLifecycleState(state);
    if (action != null) {
      widget.store.send(action);
    }
  }

  @override
  void didChangeMetrics() {
    var action = widget.observer?.didChangeMetrics();
    if (action != null) {
      widget.store.send(action);
    }
  }

  @override
  void didChangeTextScaleFactor() {
    var action = widget.observer?.didChangeTextScaleFactor();
    if (action != null) {
      widget.store.send(action);
    }
  }

  @override
  void didHaveMemoryPressure() {
    var action = widget.observer?.didHaveMemoryPressure();
    if (action != null) {
      widget.store.send(action);
    }
  }

  @override
  Future<AppExitResponse> didRequestAppExit() {
    var action = widget.observer?.didRequestAppExit();
    if (action != null) {
      widget.store.send(action);
      return Future.value(AppExitResponse.cancel);
    } else {
      return super.didRequestAppExit();
    }
  }

  @override
  Widget build(BuildContext context) {
    return WithStore.observe(
      store: widget.store,
      observe: widget.toCupertinoAppState,
      builder: (state, send, context) => CupertinoApp.router(
        title: widget.title,
        builder: widget.builder,
        theme: widget.toThemeData?.call(state),
        locale: state.locale,
        supportedLocales: widget.supportedLocales,
        localizationsDelegates: widget.localizationsDelegates,
        routerConfig: widget.routerConfig,
        localeListResolutionCallback: widget.localeListResolutionCallback,
        localeResolutionCallback: widget.localeResolutionCallback,
      ),
    );
  }
}

mixin WidgetBindingReducer<Action> {
  Action? didChangeLocales(List<Locale>? locales) => null;

  Action? didChangePlatformBrightness(Brightness brightness) => null;

  Action? didChangeAccessibilityFeatures() => null;

  Action? didChangeAppLifecycleState(AppLifecycleState state) => null;

  Action? didRequestAppExit() => null;

  Action? didHaveMemoryPressure() => null;

  Action? didChangeTextScaleFactor() => null;

  Action? didChangeMetrics() => null;
}
