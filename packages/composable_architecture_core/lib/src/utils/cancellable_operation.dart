...
class CancelableOperation<T> {
  ...
  factory CancelableOperation.fromFuture(Future<T> result, {FutureOr Function()? onCancel}) =>
  ...
  factory CancelableOperation.fromValue(T value) => (CancelableCompleter<T>()..complete(value)).operation;
  ...
  static CancelableOperation<T> race<T>(Iterable<CancelableOperation<T>> operations) {
    ...
    for (var operation in operations) {
      operation.then((value) {
        ...
        if (!done) {
          cancelAll().whenComplete(() => completer.completeError(error, stackTrace));
        }
  ...
  Stream<T> asStream() {
    var controller = StreamController<T>(sync: true, onCancel: _completer._cancel);
  ...
  /// listeners on this operation and canceling the [onValue], [onError], and
  /// [onCancel] callbacks should not cancel the other listeners.
  CancelableOperation<R> thenOperation<R>(FutureOr<void> Function(T, CancelableCompleter<R>) onValue,
      {FutureOr<void> Function(Object, StackTrace, CancelableCompleter<R>)? onError,
      FutureOr<void> Function(CancelableCompleter<R>)? onCancel,
      bool propagateCancel = true}) {
    final completer = CancelableCompleter<R>(onCancel: propagateCancel ? _cancelIfNotCanceled : null);
        ...
        onError: onError == null
            ...
            : (Object error, StackTrace stack) async {
                ...
                try {
                  ...
                  completer.completeErrorIfPending(error2, identical(error, error2) ? stack : stack2);
                }
              });
...
class CancelableCompleter<T> {
  ...
  Future<Object?> _invokeCancelCallbacks() async {
    ...
    final cancelFutures = <Future<Object?>>[if (isFuture) toReturn, ...?_cancelForwarders?.map(_forward).nonNulls];
...
Future<void>? _forward(_CancelForwarder<Object?> forwarder) => forwarder._forward();
